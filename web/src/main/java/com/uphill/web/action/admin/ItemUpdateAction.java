	...
	public ViewResolver execute(HttpServletRequest request, HttpServletResponse response) {
		...
		List<ItemOptionVO> itemOptionList = new ArrayList<ItemOptionVO>();
 		try {
			...
			int itemIndex = Integer.parseInt(multipartRequest.getParameter("itemIndex"));
			...
			itemVO = new ItemVO(itemIndex, itemCategoryIndex, itemCategoryName, itemSubcategoryIndex, itemSubcategoryName, itemName, fileSystemName, itemPrice, itemDiscountPrice, itemTotalQty);
			...
			for(int i = 0; i < itemOptionNameArray.length; i++) {
				itemOptionList.add(new ItemOptionVO(itemIndex, itemOptionNameArray[i], itemOptionContentArray[i], Integer.parseInt(itemOptionPriceArray[i]), Integer.parseInt(itemOptionQtyArray[i])));
			}
 		...
 		adminService.updateItem(itemVO, itemOptionList);
 		
		return new ViewResolver("/admin/item-info?itemIndex=" + itemVO.getItemIndex(), true);
	}
